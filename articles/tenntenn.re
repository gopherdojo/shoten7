...
筆者がISUCON9の予選に参加した際、事前に次のような準備を行ないました。
...
そのため、それらを用いて練習を行ないます。
その際に、チームメンバーの役割を決めておくと良いでしょう。
...
たとえば、Fabric@<fb>{fabric}のようなリモートでコマンドが実行可能なタスクランターを用意したり、
...
ソースコードを入力データとし、バグの発見やコード生成などを行ないます。

静的解析は@<img>{static-analysis-flow}のような流れで行われ、
...
これらの情報を用いてバグの発見やコード生成を行ないます。

=== 静的解析とGo

Goでは次のような理由から容易に静的解析が行える環境が整っています。
...
しかし、ソースコードに特定の処理を埋め込んでいったり、ソースコードからSQL文を取り出すには、
専用の静的解析ツールを自作する必要があります。
...
@<code>{defer}文を用いることで1文で書くことができます。
...
計測した結果は@<list>{measurestats}のように取得することができます。
...
同様に無名関数を表す@<code>{*ast.FuncLit}型のノードに対しても処理を行ないます。
...
関数の先頭に計測用の関数呼び出しを差し込んでいくだけではビルドが通りません。
...
文字列定数といっても@<code>{"SELECT * " + "FROM items"}のような定数式になっているかもしれません。
...
@<code>{types.Eval}関数によって定数式として評価を行ないます。
...
一方、次のフェーズの型チェックまで行うと定数式の評価も行なえますが、
...
本稿では静的解析を用いてISUCON向けのツールを作る方法について解説を行ないました。
