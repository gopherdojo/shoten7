...
自分はソフトウェアエンジニアないしGoをはじめて約1年になります。プログラミングとしては多少触れていたのですが、
...
Go1.9からt.Helperが追加されるようになりました。testing内でt.Helperを一行書いておくとテストエラーの箇所がわかりやすくなります。
...
たとえば、外部ライブラリを利用してエラーハンドリングをエラー文で判定していた場合、ライブラリアップデートでエラー文が微妙に変わったらそれだけでテストが失敗してしまいます。
